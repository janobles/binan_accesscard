...
1. Track subsidy distribution per **batch** (one distribution event, e.g. one day
...
3. **Admin overview:** Admin/Developer see per-scanner performance and batch
   totals.
4. Split the scan flow: a **distribution-setting page** (pick subsidy type)
...
- **No open batch ⇒ scanning blocked:**
  - Setting page renders a "No active distribution - ask an administrator"
    state instead of the subsidy-type picker.
...
| Route | Action |
|---|---|
| `GET scanner/setting` | Distribution-setting page: active batch info (name, date, running total) + subsidy-type picker → "Start scanning" |
...
- New `app/Views/Scanner/kiosk-layout.php` - minimal full-viewport shell, no
  sidebar/topbar. Slim header bar: batch name · subsidy-type badge ·
...
- The subsidy-type dropdown and its session-JS logic are **removed** from
  `scan.php`; the subsidy type comes from the setting page (server-validated
...
- **Admin/Developer view:** batch summary cards (families served, total
  handouts, coverage) + per-scanner table + existing barangay/subsidy-type
...
| `Controllers/Scanner/ScanController` | New `setting()` action; `scan()` guards subsidy-type + open batch; `logAid()` stamps batch, returns `myBatchCount` |
...
- `ScanController`: scan redirects to setting without subsidy type/open batch;
