...
Reports slice subsidy distribution by subsidy type, barangay, and received/not-received.
Those charts are meaningless on thin data with only three hardcoded subsidy types.
...
- **Manage** - aggregated, row-level records across all families. Search a
  family, filter, and correct (void) individual claims; manage subsidy types.
...
1. **Scan tab: log-in-place + identity enrichment.** Logging a distribution
   ...
   family's subsidy history continues to show inline.
2. **Manage tab: global distributions + subsidy types.** A back-office hub with
   two sections: (a) a searchable/filterable table of **all** aid distributions
   with a **void** action, and (b) **Subsidy Types CRUD**.
...
- **Move logging into the Scan tab.** After a successful lookup, the scan panel
  shows the log-aid form inline (claimant dropdown from `familyMembers`, subsidy-type
...
- **Model:** extend `App\Models\Scanner\AidDistributionModel` with a global
  ...
  table uses.
- **View:** a DataTable of every claim: family head, claimant, subsidy type, claim
...
- **Model - `AidTypeModel` (extend existing):** add alongside `active()`:
  ...
  - `delete(int $id): bool` - hard delete; controller/UI must refuse when the
    ...
    in history.
- **View:** table of subsidy types (name + status) + "Add" modal + per-row
...
rest of the module inlines). Every mutation (subsidy-type create/archive/restore/
...
POST scanner/aid-types/create                -> subsidy-type create
POST scanner/aid-types/archive/(:num)        -> subsidy-type archive/$1
POST scanner/aid-types/restore/(:num)        -> subsidy-type restore/$1
POST scanner/aid-types/delete/(:num)         -> subsidy-type delete/$1
...
matter are **name, relationship, birthday, sex, and subsidy history** (shown in the
...
- Every mutation (subsidy-type CRUD, distribution void) writes an `audit_trails`
...
- Run `vendor/bin/phpunit` before and after. Smoke-test: (1) Scan a registered
  ...
  family's history updates; (2) Manage → add a subsidy type → it appears in the scan
...
- Aid distribution **Reports** (charts, %, barangay selectors, report
  generation) - the read-only analytics half, unblocked by managed subsidy types and
