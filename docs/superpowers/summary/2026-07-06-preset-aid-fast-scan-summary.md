# Pre-set Subsidy Type + Fast-Scan Distribution - Implementation Summary
...
The Scanner **Scan** screen now sets the subsidy type **once** at the top, then logs
each family's handout with a single **Confirm** tap - removing the per-scan
subsidy-type dropdown that bottlenecked the point-of-service queue.

- **Sticky pre-set selector** (`#sessionAidType`) at the top of the Scan screen.
  Lookup/scan is blocked until a subsidy type is chosen (inline hint otherwise).
...
- **Duplicate guard (client-side):** if the family already claimed the pre-set
  subsidy type **today**, a warning banner shows and Confirm turns yellow - override
  is allowed and deliberate. Matching is by `aid_type_id` (subsidy-type names are not
...
- **Fast reset:** after a successful log the panel clears, the control input
  refocuses, `claim_date` resets to today, and the subsidy type stays set - ready
  for the next family.
