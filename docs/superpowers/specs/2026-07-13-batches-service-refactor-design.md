...
to a service, not a subsidy type. The `admin/distribution` page also crams
three tabs (Subsidy Types / Batches / All Distributions) into one page with
...
- Delete `AidTypeModel`, subsidy-type CRUD in `Admin\DistributionController`,
  `distribution-aidtypes-body.php`, subsidy-type routes.
...
- Kiosk header badge: service shortcode + name (category code as small
  text) instead of subsidy type.
- `AidStatsModel`, `admin/reports`, `ReportsPdfGenerator`, `pdf/report.php`:
  subsidy-type columns → service/category (with codes).
