...
camera), pull up the family + prior subsidy history, and log a new aid
...
| Aid types | `app/Models/Scanner/AidTypeModel.php` | `active(): array` - non-archived (`dt_deleted IS NULL`) subsidy types for the log-aid dropdown. |
...
null). These populate the log-aid dropdown out of the box; subsidy-type CRUD
